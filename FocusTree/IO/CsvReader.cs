...
namespace FocusTree.IO
{
    internal class CsvReader
    {
        ...
        public static FocusGraph LoadGraph(string path)
        {
            ...
            Dictionary<int, FocusData> NodeCatalog = new();
            Dictionary<int, List<HashSet<int>>> RequireGroups = new();
            ReadGraphFromCsv(path, ref NodeCatalog, ref RequireGroups);
            return new FocusGraph(Path.ChangeExtension(path, ".xml"), NodeCatalog, RequireGroups);
        }
        private static void ReadGraphFromCsv(string path, ref Dictionary<int, FocusData> nodes, ref Dictionary<int, List<HashSet<int>>> requires)
        {
            ...
            foreach (var row in data)
            {
                ...
                if (level == last.Level + 1)
                {
                    ...
                    requires.Add(newNode.Value, new List<HashSet<int>> { new HashSet<int> { last.Value } });
